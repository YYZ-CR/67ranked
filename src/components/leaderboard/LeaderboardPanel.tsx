...
const RefreshIcon = ({ spinning = false }: { spinning?: boolean }) => (
  <svg className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${spinning ? 'animate-spin' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
export function LeaderboardPanel({ refreshTrigger, onMinimize, showMinimizeButton = false }: LeaderboardPanelProps) {
  ...
  return (
    <div className="h-full flex flex-col glass-panel rounded-lg sm:rounded-xl overflow-hidden">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {isLoading ? (
          ...
          <div className="p-3 sm:p-4 text-center">
            <p className="text-white/40 text-[10px] sm:text-xs">No scores yet</p>
          </div>
        ) : (
          <div>
      ...
      {/* Footer */}
      <div className="flex-shrink-0 px-2 sm:px-3 py-1.5 sm:py-2 border-t border-white/5">
        <span className="text-[9px] sm:text-[10px] text-white/30 font-mono">
...
function LeaderboardRow({ rank, username, score, isTop3, index, is67RepsMode = false }: LeaderboardRowProps) {
  ...
  return (
    <div 
      className={`
        flex items-center px-2 sm:px-3 py-1 sm:py-1.5 transition-colors border-b border-white/5 last:border-b-0
    ...
    >
      ...
      <div className="w-5 sm:w-6 flex-shrink-0">
        <span className={`text-[10px] sm:text-xs font-bold ${getRankColor()}`}>
          {rank}
        </span>
      </div>
      ...
      <div className="flex-1 min-w-0 mr-2">
        <p className={`truncate text-[10px] sm:text-xs ${isTop3 ? 'text-white font-medium' : 'text-white/70'}`}>
          {username}
        </p>
      </div>
  ...
  return (
    <div>
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="flex items-center px-2 sm:px-3 py-1 sm:py-1.5 border-b border-white/5">
          <div className="w-5 sm:w-6 flex-shrink-0">
            <div className="w-3 sm:w-4 h-2.5 sm:h-3 bg-white/5 rounded animate-pulse" />
          </div>
          <div className="flex-1 min-w-0 mr-2">
            <div className="h-2.5 sm:h-3 bg-white/5 rounded w-12 sm:w-16 animate-pulse" />
          </div>
          <div className="flex-shrink-0">
            <div className="h-2.5 sm:h-3 bg-white/5 rounded w-6 sm:w-8 animate-pulse" />
          </div>
        </div>
      ))}
    </div>
